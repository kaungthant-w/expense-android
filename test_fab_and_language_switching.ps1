...
Write-Host "  b) Test Language Switching - go to Settings > Language and change language" -ForegroundColor Cyan
Write-Host "  c) Check if Navigation Drawer, Today's Summary, and TabLayout update language" -ForegroundColor Cyan
Write-Host "`nWatching logs for 60 seconds..." -ForegroundColor Yellow
...
Start-Job -ScriptBlock {
    ...
    Start-Sleep $timeout
} -ArgumentList $timeout | Wait-Job -Timeout ($timeout + 5) | Receive-Job
...
Write-Host "4. Today's Summary card updates language" -ForegroundColor White
