...
class MainActivity : BaseActivity(), NavigationView.OnNavigationItemSelectedListener {
    ...
    private val allListActivityLauncher = registerForActivityResult(
      ...
      override fun onCreate(savedInstanceState: Bundle?) {
        try {
            ...
            // Initialize Firebase services and check for festival messages
    ...
    private fun initializeFirebaseServices() {
        ...
        // Fetch festival message from Remote Config
        fetchFestivalMessageFromRemoteConfig()
        
        // Check festival message from Firestore
        checkFestivalMessageFromFirestore()
        
        // Check festival message from Realtime Database
        checkFestivalMessageFromRealtimeDatabase()
    }

    private fun checkNotificationIntent() {
        val festivalMessage = intent.getStringExtra("festival_message")
        val festivalAnimation = intent.getStringExtra("festival_animation")
        val festivalName = intent.getStringExtra("festival_name")

        if (!festivalMessage.isNullOrEmpty()) {
            // Create enhanced message with festival name if available
            val enhancedMessage = if (!festivalName.isNullOrEmpty()) {
                "$festivalName\n\n$festivalMessage"
            } else {
                festivalMessage
            }

            val animationResId = when (festivalAnimation) {
                "fireworks" -> R.raw.fireworks
            ...
            intent.removeExtra("festival_animation")
            intent.removeExtra("festival_name")
        }
    }

    private fun fetchFestivalMessageFromRemoteConfig() {
        ...
        remoteConfig.fetchAndActivate()
            .addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    ...
                    val animation = remoteConfig.getString("animation")
                    val festival = remoteConfig.getString("festival")

                    if (message.isNotEmpty()) {
                        // Create enhanced message with festival name if available
                        val enhancedMessage = if (!festival.isNullOrEmpty()) {
                            "$festival\n\n$message"
                        } else {
                            message
                        }

                        val animationResId = when (animation) {
                            "fireworks" -> R.raw.fireworks
                            else -> null
                        }
                        val title = if (!festival.isNullOrEmpty()) "🎉 $festival" else null
                        showFestivalDialog(enhancedMessage, animationResId, title)
                    }
                } else {
                    Log.w("MainActivity", "Failed to fetch remote config", task.exception)
                }
            }
    }

    private fun checkFestivalMessageFromFirestore() {
        ...
        db.collection("festival")
            ...
            .addOnSuccessListener { document ->
                if (document.exists()) {
                    ...
                    val festival = document.getString("festival")
                    val message = document.getString("message")
                    val animation = document.getString("animation")

                    // Check if the festival is for today
                    val today = SimpleDateFormat("yyyy-MM-dd", Locale.getDefault()).format(Date())
                    if (date == today && !message.isNullOrEmpty()) {
                        // Create enhanced message with festival name
                        val enhancedMessage = if (!festival.isNullOrEmpty()) {
                            "$festival\n\n$message"
                        } else {
                            message
                        ...
                        showFestivalDialog(enhancedMessage, animationResId, "🎉 $festival")
                    }
                }
            }
            .addOnFailureListener { e ->
                Log.w("MainActivity", "Error getting festival document", e)
            }
    }

    private fun checkFestivalMessageFromRealtimeDatabase() {
    ...
    private fun showFestivalDialog(message: String, animationResId: Int? = null, title: String? = null) {
        ...
        val festivalMessage = dialogView.findViewById<TextView>(R.id.festivalMessage)
        val animationContainer = dialogView.findViewById<FrameLayout>(R.id.animationContainer)
        val btnClose = dialogView.findViewById<Button>(R.id.btnClose)
        val btnCelebrate = dialogView.findViewById<Button>(R.id.btnCelebrate)

        // Set custom title if provided, otherwise use default
        festivalTitle.text = title ?: "🎉 ပွဲတော်နေ့"
        festivalMessage.text = message
        ...
        btnCelebrate.setOnClickListener {
    ...
    private fun showCelebrationAnimation() {
        // Create a full-screen celebration animation
        val animationView = LottieAnimationView(this).apply {
            setAnimation(R.raw.fireworks)
        ...
        // Add to the root view temporarily
